...
class Network(object):
  ...
  def test_image_2(self, sess, image, im_info, output_layers):
    feed_dict = {self._image: image,
                 self._im_info: im_info}
    
    fetches = {}
    for layer_name in output_layers:
      var_name = layer_name.net_layer(self._resnet_scope)
#       key_name = layer_name.net_layer_weights(self._resnet_scope)
      fetches[var_name] = self._predictions[var_name]
#       name = self._resnet_scope + '/' + name
#       fetches[name] = self._predictions[name]
